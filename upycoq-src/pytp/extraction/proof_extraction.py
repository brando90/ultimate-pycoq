...
def format_proof_text(proof_text, proof_term_only=False):
    ...
    proof_texts = [each.strip() + '. ' if i != len(proof_text.split('. ')) - 1 else each.strip() + ' ' for i, each in enumerate(proof_text.split('. ')) ]
    ...
    proof_texts = filter(lambda x: x.strip() != '' or x.strip() != '.', proof_texts)
...
def get_augmented_proof_steps(formated_proof_text):
    ...
    augmented_proof_steps = [proof_steps + ' Show Proof.' for proof_steps in proof_steps[:]] # + [proof_steps[-1]]  # the first Show Proof will always be '?Goal', TODO: check this
...
def substitute_augmented_proof_steps(text_lines, augmented_proof_steps, proof_start_line, proof_start_char, proof_end_line, proof_end_char):
    ...
    text_lines[proof_start_line] = text_lines[proof_start_line][:proof_start_char] + ' '.join(augmented_proof_steps) + '\n' 
            ...
            if document_symbols:
                ...
                for k, v in theorems.items():
                    if not v['only_proof_term']:
                        ...
                        theorems[k]['augmented_proof'] = get_augmented_proof_steps(format_proof_text(v["proof"]))
                        ...
                        for i in range(0, len(checkpoints)):
                            if is_statement:
                                ...
                                proof_term = client.wait_for_response(id, timeout=3)
...
def winston_coq_lsp():
    ...
    new_workspace_path = Path('/home/jizej/proverbot9001/coq-projects/markov').expanduser()
    ...
    for k, v in theorems.items():
        ...
        print("===========")
        print(f'Formated Proof: {format_proof_text(v["proof"], v["only_proof_term"])}')
        print("===========")
        if not v["only_proof_term"]:
            new_thm_text = "".join(text_lines[v["def_ranges"][0]:(v["proof_ranges"][2]+1)])
            print(v["def_ranges"][0],v["proof_ranges"][2], f'Original Text: {new_thm_text}')
            text_lines = substitute_augmented_proof_steps(text_lines, v['definition'].split('\n'), v['def_ranges'][0], 0, *v['def_ranges'][2:])
            text_lines = substitute_augmented_proof_steps(text_lines, get_augmented_proof_steps(format_proof_text(v["proof"])), v['proof_ranges'][0], 0, *v['proof_ranges'][2:])
            new_thm_text = "".join(text_lines[v["def_ranges"][0]:(v["proof_ranges"][2]+1)])
            print(v["def_ranges"][0],v["proof_ranges"][2], f'Augmented Text: {new_thm_text}')
            print("===========")
            checkpoints = mark_checkpoints_in_range(text_lines, v['proof_ranges'][0],0,v['proof_ranges'][2],-1)
            ...
            for i in range(0, len(checkpoints)):
                if i == 0:
                    print(text_lines[checkpoints[i][0]][:(checkpoints[i][1]+1)])
                else:
                    print(text_lines[checkpoints[i][0]][(checkpoints[i-1][1]+1):(checkpoints[i][1]+1)], is_statement)
                if is_statement:
                    proof_steps['text'] = format_proof_text(v['proof']).split('\n')[curr_idx]
